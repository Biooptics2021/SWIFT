...
center=[1317,1085];
for ii=1:36
images(:,:,ii)=loadtiff(['demo_data\demo_data_1second_frame' num2str(ii) '.tif']);
end
...
ObjRECON_t_group2=zeros(size(image_seg,1),size(image_seg,2),81,times,'single'); 
...
for ii=1:size(psfWAVE_STACK,3)
   ...
   for views=1:4
       ...
       delta2=squeeze(delta_mm_g2(views,:,:));
       center=mean(delta2,2)*6.5;
...
%       delta1=squeeze(delta_mm_g1(views,:,:));
       delta2=squeeze(delta_mm_g2(views,:,:));
       center=mean(delta2,2)*6.5+move1(time,:)*1000;
       center=center/(150/33)+[3302,3302];
       center=round(center);
       lens=zeros(6603,6603);
...
imagepower2(:,time)=gather(image_power_z1);
toc;
ObjRECON_t_group2(:,:,:,time)=gather(ObjRecon1);

end

ObjRECON_t_group2=uint16(round(65535.*ObjRECON_t_group2./max(ObjRECON_t_group2(:))));
 save ([savepath, 'ObjRecon_parts2.mat'], 'ObjRECON_t_group2','imagepower2', '-v7.3');
